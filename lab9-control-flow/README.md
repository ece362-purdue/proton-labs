...
### 0.1: Assembly directives and labels

You already worked with assembly directives when working on the previous lab. In the `lab.S` file, any line starts with `.` is an assembly directive. These directives instruct the assembler on how to organize the binary and hint to the linker what can be made visible to other object files. Basically, directives are like C macros, including some meta-information beyond the actual program.  Below is a short list of the common directives in RISC-V:
...
For instance, in the last lab, the label `lowercase_string:` was a synonym to the address of the string `"ece 362 is awesome!"`. 
...
### 0.2 Control Flow Assembly Instructions
...
### 0.2.4 Creating higher-level constructs: if-else
...
### 0.2.5 Creating higher-level constructs: Loops
