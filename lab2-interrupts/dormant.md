...
1. Before entering DORMANT mode, the code changed the source of the system clock to be the crystal oscillator directly, instead of the System PLL.
...
2. They enter DORMANT mode and wait for a rising edge trigger to wake the microcontroller, and wait for the XOSC to become stable again.

3. Upon waking, the function that initializes PLLs and other clocks is called to restore the original system clock configuration, restoring the original system clock frequency.  We also call `stdio_uart_init` again to ensure the UART has picked up the new frequency, so that you can continue to see "Hello world" to print.
...
Hopefully, that should allow you to resume printing "Hello world" when you wake up from DORMANT, and be able to enter and exit DORMANT multiple times.  This functionality will be extremely useful for low-power projects, such as smartwatches and other wearable projects.
